...
app.config.update(dict(
    ...
    RECAPTCHA_SITE_KEY = "6LfKTxAbAAAAAOCkZcNQfNQgpLn8WYrQtu-vIzPj",
    RECAPTCHA_SECRET_KEY = "6LfKTxAbAAAAADePX9iC8vGX5FYqCY2EYDS_Y0xO"
    ))
...
def otp():
    ...
    if request.method == 'POST' and 'hotp' in request.form and 'totp' in request.form:
        ...
        template = render_template('ex.html', one=auth.HOTP(key,counter_list[-1]), two = auth.TOTP(key), user = session['username'])
        msg = Message('OTP', sender = 'mailforproject333@gmail.com', html = template, recipients = [session['email']])
        mail.send(msg)
